...
export class InstallerService {
  async getMyRoute(userId: number, date?: string) {
    // 1. Find InstallerProfile by userId
    const installerProfile = await prisma.installerProfile.findUnique({
      where: { userId },
      include: {
        user: { select: { fullName: true } },
        zone: { select: { id: true, nameHe: true } },
      },
    });
    if (!installerProfile) {
      throw new AppError(404, 'NO_INSTALLER_PROFILE', 'לא נמצא פרופיל מתקין');
    }

    // 2. Find the Route for this installer on the target date
    ...
    const installer = {
      name: installerProfile.user.fullName,
      department: installerProfile.department,
      zone: installerProfile.zone,
      startTime: installerProfile.startTime,
      endTime: installerProfile.endTime,
    };

    if (!route) {
      return { installer, route: null, orders: [] };
    }
  ...
  async recordDelivery(
    ...
    // Verify installer profile exists
    const installerProfile = await prisma.installerProfile.findUnique({
      where: { userId },
    });
    if (!installerProfile) {
    ...
    // Verify installer is assigned to this route
    if (order.route.installerProfileId !== installerProfile.id) {
  ...
  async uploadSignature(userId: number, orderId: number, signatureBase64: string) {
    const installerProfile = await prisma.installerProfile.findUnique({ where: { userId } });
    if (!installerProfile) throw new AppError(403, 'FORBIDDEN', 'אין לך הרשאה לפעולה זו');
